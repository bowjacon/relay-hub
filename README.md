...
- 请求日志会记录不含正文的缓存前缀 SHA-256 指纹、前缀大小和相邻请求是否变化，用于排查缓存命中率；中转不会向 `messages`、`input`、`system` 或工具定义注入时间戳。
- 对 OpenAI-compatible 来源可在服务端 `.env` 设置稳定的 `RELAY_PROMPT_CACHE_KEY`（未设置时第三方来源默认使用 `relay-hub`）。中转会为每个来源、模型和 Agent 生成稳定的 `prompt_cache_key`，并对 Claude→OpenAI 请求移除不适用的 `cache_control` 标记、规范化 JSON 字段顺序；若上游不支持该字段，会自动重试一次并停用该来源的缓存键。
